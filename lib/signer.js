...
var Signer = function (rsa)
{
  if (rsa === undefined)
    rsa = config.rsa;  
  if ("object" !== typeof rsa)
    throw UserError('Config error: rsa is not an object');

  if ("string" !== typeof rsa.d)
    throw UserError('Config.error: rsa.d is not a string');

  if ("string" !== typeof rsa.e)
    throw UserError('Config error: rsa.e is not a string');

  this.e = bignum(rsa.e, 16);
  this.d = bignum(rsa.d, 16);
  this.phi = bignum(rsa.phi, 16);
  this.n = bignum(rsa.n, 16);
};
